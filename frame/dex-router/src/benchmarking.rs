...
benchmarks! {
  where_clause { where
	  ...
	  <T as pallet_dex_router::Config>::Balance: From<u128>, <T as pallet_dex_router::Config>::AssetId: From<u128>, <T as pallet_dex_router::Config>::PoolId: From<u32>,
  }
  ...
  update_route {
	...
	assert_ok!(&p);
	// let stable_swap_pool_id : u128 = p.unwrap().into();
	...
	assert_ok!(&p);
	// let uniswap_pool_id = p.unwrap();
	let dex_route = vec![
			DexRouteNode::Uniswap(0_u32.into()),
			DexRouteNode::Curve(0_u32.into()),
	];
  ...
  exchange {
	...
	sp_std::if_std! {
		println!("stable_swap_pool_id {:?}", stable_swap_pool_id);
	}
	...
	sp_std::if_std! {
		println!("uniswap_pool_id {:?}", uniswap_pool_id);
	}
	let eth_amount = 1_000_000_000_u128 * unit;
	let usdc_amount = eth_amount * 3000_u128 * unit;
	...
	pallet_dex_router::Pallet::<T>::update_route(origin.into(), currency_pair, Some(dex_route.clone().try_into().unwrap())).expect("insert route failed");
